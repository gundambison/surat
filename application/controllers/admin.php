...
class Admin extends CI_Controller {
	...
	public function klas_surat() {
		...
		if ($mau_ke == "cari") {
			...
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data telah diperbaharui</div>");			
	...
	public function surat_masuk() {
		...
		/* pagination */	
		$total_row		= $this->db->query("SELECT * FROM t_disposisi, t_surat_masuk WHERE YEAR(tgl_diterima) = '$ta'")->num_rows();
		$per_page		= 10;
		...
		$config['allowed_types'] 	= 'gif|jpg|png|pdf|doc|docx|rar|zip|xlsx|xls|ppt|pptx';
		...
		if ($mau_ke == "del") {
			...
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data telah ditambahkan ".$this->upload->display_errors()."</div>");
			redirect('index.php/admin/surat_masuk');
		} else if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
	...
	public function surat_keluar() {
		...
		if ($mau_ke == "del") {
			...
			$a['data']		= $this->db->query("SELECT * FROM t_surat_keluar WHERE isi_ringkas LIKE '%$cari%' OR tujuan LIKE '%$cari%' OR no_surat LIKE '%$cari%' OR kode LIKE '%$cari%' OR no_agenda LIKE '%$cari%' OR tgl_surat LIKE '%$cari%' ORDER BY id DESC")->result();
			...
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data telah ditambahkan</div>");
			redirect('index.php/admin/surat_keluar');
		} else if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
	...
	public function do_login() {
        ...
        if($j_cek == 1) {
			...
			$this->session->set_flashdata("k", "<div id=\"alert\" class=\"alert alert-error\">username atau password tidak ditemukan</div>");
			redirect('index.php/admin/login');
		}
	}
